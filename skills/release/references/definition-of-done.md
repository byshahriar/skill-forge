...
A standing, project-wide bar that every change must clear before it counts as done. Unlike acceptance criteria, which vary per task and answer "did we build the right thing?", the Definition of Done is the same every time and answers "is this finished to our standard?". Use it as the final gate in `plan`, `implement`, and `release`.
...
The depth behind these items lives in `code-review` (the five-axis review) and `refactor` (reducing complexity without changing behavior).
...
- [ ] Architectural decisions worth preserving are recorded (see `docs`)
- [ ] Documentation describes the current state in timeless language, not the change history

### Ship-readiness
- [ ] Security implications reviewed for any untrusted input, auth, or data handling (see `security`)
- [ ] Observability in place for new critical paths (logs, metrics, traces) (see `observability`)
- [ ] Rollback path exists for anything risky (see `release`)
...
- **Per release**: the full checklist is the floor; `release` adds the deploy-specific gates on top.
