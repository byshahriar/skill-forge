...
Beyond challenging others' claims, apply the same discipline to your own — the iron law:

```
NO COMPLETION CLAIMS WITHOUT FRESH VERIFICATION EVIDENCE
```

If the verification command wasn't run *in this message*, the claim cannot be made. The gate, before any "done/fixed/passing":

1. **Identify** — what command proves this claim?
2. **Run** — the full command, fresh (not a cached or earlier run)
3. **Read** — the whole output: exit code, failure counts, warnings
4. **Verify** — does the output actually confirm the claim? If no: state the real status with the evidence. If yes: state the claim *with* the evidence.
...
| Linter clean | Linter output: 0 errors | Extrapolating from a partial check |
...
| Requirements met | Line-by-line checklist against the spec | "Tests pass, so we're done" |

Language tells: "should work", "probably", "seems to", or celebrating ("Perfect!") before the verification ran — all mean the gate was skipped. Rewording the claim doesn't exempt it; spirit over letter.
...
- **`code-research`**: SDD verifies *facts about frameworks* against official docs. Doubt-driven verifies *your reasoning about the artifact*. SDD checks the API exists; doubt-driven checks you used it correctly under the contract.
...
- **`debug`**: when the reviewer surfaces a real failure mode, drop into the debugging skill to localize and fix.
