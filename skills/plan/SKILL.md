...
## The Zero-Context Standard

Write every plan as if the executor is a skilled developer who knows **nothing about this codebase or problem domain** — because the executor often is exactly that (a fresh agent session, a subagent, a new teammate). The litmus test for every task: could someone with zero context complete it using only what's written?

That means each task carries:

- **Files** — exact paths to create/modify (with line ranges for modifications), and the test file
- **Interfaces** — what this task *consumes* from earlier tasks and *produces* for later ones, as exact signatures. A task's implementer may see only their own task; this block is how they learn the names and types their neighbors use.
- **Steps as single actions** (2–5 minutes each), checkbox-tracked: write the failing test (shown) → run it, expect FAIL with this message → minimal implementation → run, expect PASS → commit. The expected command output is written down — "run the test" without the expected result isn't verifiable.

**Plan header** — every plan starts with: goal (one sentence), architecture (2–3 sentences), tech stack, a link to the spec it implements (the plan argues *from* the spec, so the spec travels with it), and **Global Constraints** — the spec's project-wide requirements (version floors, naming rules, platform requirements) copied verbatim, one line each; every task implicitly includes them.

**File structure before tasks.** Map which files will exist and what each is responsible for *before* decomposing into tasks — this is where decomposition decisions get locked in. One clear responsibility per file; follow the codebase's existing patterns.

**Task right-sizing.** A task is the smallest unit that carries its own test cycle and is worth a fresh reviewer's gate. Fold setup/scaffolding/docs into the task whose deliverable needs them; split only where a reviewer could reject one task while approving its neighbor. If the spec covers multiple independent subsystems, that's multiple plans — each producing working, testable software on its own.
