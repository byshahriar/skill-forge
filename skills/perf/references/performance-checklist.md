...
Quick reference checklist for web application performance. Use alongside the `perf` skill.
...
The decision material (which layer, which invalidation strategy, what never to cache) lives in the `perf` skill. This section covers the read/write patterns and the checklist.
