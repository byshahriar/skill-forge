...
1. **Baseline** — measure the key flows (page loads, hot endpoints, critical queries) under controlled conditions; store results with date and commit hash (`.perf/baseline.json`)
2. **Compare on demand or per release** — same flows, same conditions; report relative change per flow
3. **Alert on relative regressions, not absolutes** — 2x baseline is a regression; 1.5x may be variance; judge against *your* baseline, not industry numbers
4. **Confirm before crying wolf** — re-run a regressed measurement; one noisy sample is not a finding
5. **Update the baseline deliberately** — after an accepted change in performance character, promote the new numbers; never silently, or the ratchet is lost
