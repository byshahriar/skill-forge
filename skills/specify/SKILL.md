...
- **Interfaces live at the boundary.** The map records that `billing` depends on `identity`; the contract between them belongs in the provider module's spec (see `api-design` for designing it).
...
> Follow `plan` for the dependency-graph mapping and vertical-slicing mechanics behind these steps; it is the canonical source. The bullets above are a lightweight summary; if they ever diverge, `plan` takes precedence.
>
> **Output convention:** Save the plan to `tasks/plan.md` and record the task list in the task list target defined by `plan` (default `tasks/todo.md`; projects may designate an external tracker instead). Create `tasks/` if it does not exist. Downstream commands (`/build`, etc.) expect these defaults.
...
> Follow `plan` for the full task-sizing and dependency-ordering mechanics; it is the canonical source. The template below is a lightweight inline form; if they ever diverge, `plan` takes precedence.
...
Execute tasks one at a time following `skills/implement/SKILL.md` (`implement`) and `skills/tdd/SKILL.md` (`tdd`). Use `skills/context/SKILL.md` (`context`) to load the right spec sections and source files at each step rather than flooding the agent with the entire spec.
...
A spec an engineer can execute without questions carries these, whichever apply:

1. **Stakeholder context** — why this matters, in the requester's terms; the spec's decisions trace back to it
2. **Verified current state** — claims about existing behavior verified against the code, with file references, not remembered ("read code first" is a hard requirement: interrogate the actual implementation before writing constraints)
3. **Audit tables** — for work spanning many call sites: a table of every site, its current state, and its target state; the table *is* the scope
4. **Quantified impact** — numbers where they exist (rows affected, req/s, users on the old path), estimates labeled as estimates
5. **Prioritized recommendations with rationale** — when the spec chooses between options, the losers and the why stay in the doc
6. **"Do not touch" list** — what's working well and explicitly out of bounds; protects the spec from helpful drive-by refactors
7. **Dependency graph** — for multi-part work, what blocks what (ASCII is fine)
8. **Schemas and API shapes** — exact field names and types for anything crossing a boundary
9. **File reference table** — file → role in this change; the implementer's map
10. **Testable acceptance criteria** — each one checkable by someone who didn't write the spec
