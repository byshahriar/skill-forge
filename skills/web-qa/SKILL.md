...
## The QA Sweep: Test → Fix → Verify

For "QA the app" requests (vs debugging one issue), run the systematic sweep:

1. **Plan the pass** — enumerate pages/flows to test (nav crawl or user-provided list), prioritizing critical paths; decide fix mode (find-and-fix) or report-only up front
2. **Baseline pass** — per page: load, screenshot, console errors, obvious visual breakage, primary interaction works. Log every anomaly with evidence (screenshot + console output) before fixing anything.
3. **Triage** — dedupe and rank findings: broken flows > errors > visual defects > polish; estimate blast radius per fix
4. **Fix loop** (fix mode) — per finding: locate source (grep the error/component/route) → smallest fix → **re-test the exact reproduction** → also re-test adjacent flows the fix could touch → commit with the finding reference
5. **Regression test** — every fixed bug gets a test that fails without the fix, where the project's test setup allows
6. **Self-regulation stop** — after every 3 fixes, step back: are fixes revealing new problems elsewhere? That's a wrong-layer signal — stop patching and escalate to `debug` for root cause
7. **Report** — findings table (fixed / open / won't-fix with reasons), evidence paths, and what was *not* tested
