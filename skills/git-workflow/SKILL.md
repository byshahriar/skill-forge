...
The number is a promise, so make the code match it. A "patch" that changes behavior consumers relied on is a major change wearing a disguise (Hyrum's Law — see the `api-design` skill). When unsure whether a change is breaking, assume it is; a surprise major is far cheaper than a broken consumer.
...
Write the entry in the same change that makes the change, while the impact is fresh — not reconstructed from commit archaeology at release time. Breaking changes get a migration note and a deprecation window (follow the `modernization` skill); shipping the actual release is the `release` skill's job — this section is the versioning contract that feeds it.

## Worktree Isolation

For feature work that must not disturb the current workspace (or before executing an implementation plan), use an isolated worktree:

1. **Detect existing isolation first.** If `git rev-parse --git-dir` ≠ `--git-common-dir` (and you're not in a submodule — check `--show-superproject-working-tree`), you're already in a linked worktree: don't create another.
2. **Prefer the platform's native worktree tool** (an `EnterWorktree`-style tool, `/worktree` command, `--worktree` flag) — it handles placement, branch creation, and cleanup. Manual `git worktree add` alongside a native tool creates phantom state the harness can't see.
3. **Manual fallback:** use the project's existing worktree dir (`.worktrees/` preferred), and **verify it's git-ignored before creating anything** (`git check-ignore -q .worktrees`) — an unignored worktree directory ends up committed. Then `git worktree add "$path" -b "$branch"`, cd in, run project setup (install deps), and confirm the baseline tests pass *before* changing anything.
4. Ask before creating a worktree unless the user's preference is already known.

## Finishing a Branch

When implementation is complete, the close-out is a protocol, not a vibe:

1. **Green suite first.** Run the full test suite; failures mean there is no menu yet — fix or report, don't integrate red work.
2. **Confirm the base branch** — whatever this work forked from (plan, upstream, or ask). Merging into the wrong base is expensive to undo.
3. **Present exactly three options and let the user pick:** merge back to the base locally · push and open a PR · keep the branch as-is. Discarding work is never offered — it happens only when the user explicitly asks.
4. **Execute + verify:** local merge → re-run tests on the *merged* result before deleting anything (nothing's pushed; a red merge is still recoverable). PR → push, create against the confirmed base, keep the worktree for iteration on feedback. Keep-as-is → report branch and path.
5. **Cleanup last:** remove the worktree and delete the branch only after the merged result is verified green.
