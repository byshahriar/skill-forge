...
**Operating rules:**
- **Minimize and set a purpose.** Collect a field only against a stated use. "It might be useful later" is not a purpose — it's latent breach scope. Don't log PII into telemetry (the `observability` skill makes the same point from the ops side).
...
When data crosses a trust boundary, validate it as untrusted (see Input Validation above); when a privacy incident exposes personal data, the breach-notification clock is part of the postmortem — follow the `debug` skill.
...
```
## CSO Mode: Full-Codebase Audit

For "audit the whole thing" requests (vs securing one change), run chief-security-officer mode:

1. **Architecture mental model first** — map the stack, entry points, trust boundaries, and data stores before hunting bugs; findings without an architecture model are pattern-matching noise
2. **Attack-surface census** — enumerate every entry point: routes/endpoints, auth flows, file uploads, webhooks, background jobs consuming external data, third-party integrations. The census is the audit's coverage contract.
3. **Sweep by category** — work the census against the OWASP patterns above, one category at a time, recording file:line evidence for every candidate finding
4. **False-positive filtering with active verification** — before reporting, verify each finding against the live code: does the field/route/flow actually exist as claimed? Can you sketch the concrete attack path? A finding you can't walk from attacker input to impact is PLAUSIBLE, not CONFIRMED — label it.
5. **Confidence calibration** — report each finding as `Sev (crit/high/med/low) × Confidence (confirmed/plausible)` with the evidence that earned the label. Inflated findings burn the audit's credibility and bury the real ones.
6. **Report with remediation and trend** — findings table sorted by severity×confidence, concrete fix per finding, and — on repeat audits — the delta since last time (fixed / new / still open).
