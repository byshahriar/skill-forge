...
**Fix-first triage.** Classify every finding before discussing any of them:
- **AUTO-FIX** — objectively wrong, mechanical, low-risk (typo, missing await, dead import): fix directly, list what you fixed
- **ASK** — judgment calls and behavior changes: batch into one decision list for the author, each with recommendation
- **NOTE** — worth knowing, not worth blocking: record and move on

**Verify claims before asserting them.** A review comment claiming "X doesn't handle Y" must be checked against the actual code path first — reviews carry authority, and a wrong claim sends the author on a false hunt. If unverified, phrase it as a question.
...
For detailed security guidance, see `security`. Does the change introduce vulnerabilities?
...
For detailed profiling and optimization, see `perf`. Does the change introduce performance problems?
...
For triaging `npm audit` findings and supply-chain risk (typosquatting, compromised maintainers), follow the `security` skill — this section covers the upgrade *workflow*, that one covers the security verdict.
...
The other side of review: acting on feedback requires technical evaluation, not emotional performance. **Verify before implementing; ask before assuming; technical correctness over social comfort.**

The response pattern:

1. **Read** all feedback without reacting
2. **Understand** — restate each requirement in your own words (or ask)
3. **Verify** — check the claim against codebase reality before touching anything
4. **Evaluate** — technically sound *for this codebase*? (works on all platforms? breaks existing behavior? is there a reason the current code is the way it is?)
5. **Respond** — technical acknowledgment or reasoned pushback; never performative agreement ("You're absolutely right!", "Great point!") and never "implementing now" before verification
6. **Implement** — one item at a time, testing each

**Unclear items block everything.** Given items 1–6 with 4–5 unclear: don't implement 1–3 and ask later — items may be related, and partial understanding produces wrong implementations. Clarify first, then implement in order: blocking issues → simple fixes → complex fixes, testing each individually.
...
```
BEFORE approving:
1. Every changed file was actually read — not skimmed from the diff summary
2. Every claim you made was verified against the code (unverified concerns are questions, not findings)
3. Tests were read too: do they assert real behavior, and would they fail if the feature broke?
4. The change does what its description says — no more (scope drift), no less (silent gaps)
5. You could explain this change to its author's teammate. If not, you haven't reviewed it — you've witnessed it.
```
