...
## Generating Docs From Scratch

For a module or project with no docs, work from evidence, not memory:

1. **Inventory** — entry points, public API surface, configuration, invariants; read the code and tests (tests are the honest spec of current behavior)
2. **Choose the doc that pays first** — usually: a README that gets someone running in 5 minutes > API reference > architecture overview. Ask what readers fail at today.
3. **Draft against reality** — every command in the doc gets actually run; every claimed behavior gets checked against code or tests. Docs that lie are worse than no docs.
4. **Diagram the non-trivial flows** (`technical-diagrams`) — one overview diagram beats three pages of prose

## Post-Ship Doc Updates

Shipping changes documentation debt. After a release:

- Sweep the affected docs: README examples, API reference, runbooks, onboarding guides — anything naming the changed behavior
