...
This document describes the structure and format of Skill Forge skill files. Use this as a guide when contributing new skills or understanding existing ones.
...
- `license`: MIT — required on every skill for registry (skills.sh) display.
- `name` must also stay within 64 characters (registry limit); letters, numbers, and hyphens only.
...
If the build breaks, use the `debug` skill.
```
